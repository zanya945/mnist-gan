...
from keras.layers import Conv2D, MaxPool2D, Flatten, Dense, Dropout, BatchNormalization, Reshape, Input
...
class Gan():
    def __init__(self):
        ...
        self.discriminator.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])
        ...
        self.combined.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])



    def cnn_discriminator(self):
        model = Sequential()
        model.add(Conv2D(filters=6, kernel_size=(5, 5), strides=1, padding='same', input_shape=self.imgshape, activation=tf.nn.relu))
